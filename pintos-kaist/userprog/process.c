 ...
 * The new thread may be schedul	ed (and may even exit)
...
int process_exec(void *f_name)
{
	...
	*/
	...
	/* And then load the binary
		load(filname,&_if); -> load(argv[0],&_if);
		filename 전체가 아닌 첫번째 인자(프로그램 이름)으로만
	*/
	...
	char *esp = (char *)_if.rsp; // 유저 스택 최상단

	/* 문자열 복사 */
	for (int i = argc - 1; i >= 0; i--)
	{
		size_t len = strlen(argv[i]) + 1; // NULL('\0')까지 포함한 문자열 길이
		esp -= len;						  // esp를 문자열 크기만큼 낮춰, 복사할 공간 확보
		memcpy(esp, argv[i], len);		  // 스택(esp 위치)에 실제 문자열 복사
		argv[i] = esp;					  // argv[i]에 복사된 문자열의 스택 주소 저장
	}
