...
process_create_initd (const char *file_name) {
	...
	char *saveptr, *exec_file;

	exec_file = strtok_r(file_name, " ", &saveptr);

	/* Create a new thread to execute FILE_NAME. */
	tid = thread_create (exec_file, PRI_DEFAULT, initd, fn_copy);
	if (tid == TID_ERROR)
...
process_exec (void *f_name) {
	...
	memset(thread_current()->fd_list, 0, sizeof thread_current()->fd_list);
